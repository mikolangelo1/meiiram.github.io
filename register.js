document.addEventListener('DOMContentLoaded', function () {
    ...
    const commonUsernames = ['admin', 'root', 'user', 'test', 'guest'];
    ...
    function validateName(input, errorElement, fieldName) {
        ...
        if (!value) {
            ...
            error = `${fieldName} может содержать только буквы и дефис`;
        } else if (value.length < 2) {
            error = `${fieldName} должно быть не менее 2 символов`;
        } else if (value.length > 30) {
            error = `${fieldName} должно быть не более 30 символов`;
        }
    ...
    function validateEmail() {
        ...
        errors.email.textContent = error;
        inputs.email.classList.toggle('error', !!error);
        return !error;
    }
    ...
    function validateUsername() {
        ...
        if (!username) {
            ...
            error = 'Максимальная длина - 16 символов';
        } else if (!patterns.username.test(username)) {
            error = 'Используйте только буквы, цифры, дефис и подчёркивание';
        ...
        errors.username.textContent = error;
        inputs.username.classList.toggle('error', !!error);
        return !error;
    }
    ...
    function validatePassword() {
        ...
        } else if (consecutiveSymbols.test(password)) {
            error = 'Слишком много повторяющихся символов';
        }

        const strength = checkPasswordStrength(password);
        strengthProgress.setAttribute('data-strength', strength);
        ...
        errors.password.textContent = error;
        inputs.password.classList.toggle('error', !!error);
        return !error;
    }
    ...
    function validateConfirmPassword() {
        ...
        errors.confirmPassword.textContent = error;
        inputs.confirmPassword.classList.toggle('error', !!error);
        return !error;
    }

    // Валидация согласия с условиями
    function validateTerms() {
        const error = !inputs.terms.checked ? 'Необходимо согласиться с условиями' : '';
        errors.terms.textContent = error;
        return !error;
    }

    // Обработчики событий для валидации в реальном времени
    inputs.firstName.addEventListener('input', () => validateName(inputs.firstName, errors.firstName, 'имя'));
    inputs.lastName.addEventListener('input', () => validateName(inputs.lastName, errors.lastName, 'фамилию'));
    ...
    togglePassword.addEventListener('click', function () {
        ...
        this.querySelector('.icon').innerHTML = type === 'password' ?
    ...
    form.addEventListener('submit', function (e) {
        ...
        const validations = {
            firstName: validateName(inputs.firstName, errors.firstName, 'имя'),
            lastName: validateName(inputs.lastName, errors.lastName, 'фамилию'),
            email: validateEmail(),
        ...
        if (isValid) {
            ...
            // Найти первую ошибку и сфокусироваться на ней
            const firstError = Object.entries(validations).find(([, valid]) => !valid)[0];
